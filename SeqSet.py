...
class SeqSet(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        return {
            'noisy_curr' : torch.tensor(noisy_curr, dtype=torch.float32).permute(2, 0, 1), 
            'd_level' : torch.tensor(d_level, dtype=torch.float32), 
            'prev_frame' : torch.tensor(prev_frame, dtype=torch.float32).permute(2, 0, 1), 
            'delta_t' : torch.tensor(delta_t, dtype=torch.float32), 
            'noise' : torch.tensor(noise, dtype=torch.float32).permute(2, 0, 1)
        }
